--[[
    ╔══════════════════════════════════════════════╗
    ║  ZuzifyRBX Gen6 — Complete Edition           ║
    ║  45 Themes • License System • Owner Menu     ║
    ║  Supabase Sync • Trusted Program • All Feat. ║
    ╚══════════════════════════════════════════════╝
]]

--------------------------- CONFIG ---------------------------
local VERSION        = "Gen6.0.0"
...
-- Discord webhook (optional, for owner pings)
...
-- Owner / Developer UIDs (Owner sees Owner Menu; Dev can create keys)
local OWNER_UID     = 717544874
local DEVELOPER_UIDS = {} -- add numeric UIDs: {123, 456}

-- Password that "override" users can use for instant owner access (fallback)
...
local function sbGet(table_, query)
    return http("GET", SUPABASE_URL .. "/rest/v1/" .. table_ .. (query and ("?"..query) or ""), sbHeaders())
end
local function sbPost(table_, body)
    return http("POST", SUPABASE_URL .. "/rest/v1/" .. table_, sbHeaders(), body)
end
local function sbPatch(table_, query, body)
    return http("PATCH", SUPABASE_URL .. "/rest/v1/" .. table_ .. "?" .. query, sbHeaders(), body)
end

--------------------------- UTIL ---------------------------
local function DD(v) if type(v) == "table" then return v[1] or tostring(v[1]) end return v end
local function notifyText(t, c) print("[ZuzifyRBX]", t, c or "") end

-- Cache for player roles etc.
local CachedRoles = {}
...
local function showDisclaimer()
    ...
    body.Text = table.concat({
        "Before you use ZuzifyRBX, please read and accept the following:",
        "",
        "• ZuzifyRBX is a third-party script. Use is at your OWN RISK.",
        "• You must comply with Roblox Terms of Service. Using exploits may result in account action by Roblox.",
        "• ZuzifyRBX is NOT responsible for any bans, kicks, losses, or damages caused by use.",
        "• All actions you perform with this script are your own. You take full legal and moral responsibility.",
        "• Data collected: we log your UserID + username to Supabase for licensing, moderation, and anonymous statistics.",
        "• Your username is NOT publicly shown to anyone unless you enable 'Share Username' in Settings (Trusted Program).",
        "• You can request deletion of your data by contacting the owner.",
        "• License keys are non-refundable. Free tier offers limited features.",
        "• By clicking ACCEPT, you confirm you are 13+ and agree to these terms.",
    }, "\n")
    body.Parent = frame
    ...
    decline.MouseButton1Click:Connect(function()
        gui:Destroy()
        LocalPlayer:Kick("You declined the ZuzifyRBX disclaimer.")
    end)
    ...
    activate.MouseButton1Click:Connect(function()
        ...
        task.spawn(function()
            ...
            if not data or #data == 0 then
                msg.Text = "Invalid license key."
                msg.TextColor3 = Color3.fromRGB(255,120,120)
                return
            end
            local k = data[1]
            if not k.is_active then
                msg.Text = "Key is deactivated."
                msg.TextColor3 = Color3.fromRGB(255,120,120)
                return
            end
            if k.used_by and k.used_by ~= LocalPlayer.UserId then
                msg.Text = "Key already used by someone else."
                msg.TextColor3 = Color3.fromRGB(255,120,120)
                return
            end
            -- bind key
            sbPatch("zuzify_keys", "license_key=eq."..HttpService:UrlEncode(key),
                { used_by = LocalPlayer.UserId, used_at = DateTime.now():ToIsoDate() })
...
if userRow.is_banned then
    local reason = userRow.ban_reason or "No reason given."
    local until_ = userRow.ban_until and (" Ban expires: "..tostring(userRow.ban_until)) or " Permanent ban."
    LocalPlayer:Kick("ZuzifyRBX: You are banned. Reason: " .. reason .. until_)
    return
end

-- Elevate tier from server record (if previously trusted/paid)
if userRow.tier == "owner" or userRow.is_paid or userRow.is_trusted then
    if userRow.tier and userRow.tier ~= "free" and userRow.tier ~= acquiredTier then
        acquiredTier = userRow.tier
    end
end

--------------------------- FEATURE GATING ---------------------------
local function HasTier(min)
    -- min: "free", "basic", "premium", "trusted", "developer", "owner"
...
local function heartbeat()
    ...
    if not sessionId then
        local created = sbPost("zuzify_sessions", {
            ...
            is_trusted = userRow.is_trusted or false,
            tier = acquiredTier,
            job_id = JOB_ID,
        })
...
local Themes = {
    ...
    -- NEW 5 for 45 total
    ["Blood"]      = Theme(Color3.fromRGB(180,20,30),  Color3.fromRGB(14,2,4),   Color3.fromRGB(28,4,8)),
    ["Toxic"]      = Theme(Color3.fromRGB(120,255,40), Color3.fromRGB(6,14,2),   Color3.fromRGB(12,26,4)),
    ["Nebula"]     = Theme(Color3.fromRGB(200,100,255),Color3.fromRGB(10,4,26),  Color3.fromRGB(20,8,44)),
    ["Storm"]      = Theme(Color3.fromRGB(120,160,200),Color3.fromRGB(10,14,20), Color3.fromRGB(18,24,34)),
...
local Features = {
    ...
    ESP_Chams=true, ESP_Boxes=true, ESP_Tracers=false,
...
-- Sync ShareUsername to server when toggled
...
--------------------------- EMOTES (300+ built-in, no pastebin) ---------------------------
local EmoteList = {}
local function E(name, id) table.insert(EmoteList, {Name=name, ID=id}) end
local ids = {
"507770620","507771112","507771612","507771366","507771049","507771682","507771410","507771276","507771842","507771453",
"507771054","507771815","507771568","507771147","507771731","507771174","507771878","507771594","507771358","507771270",
"507771697","507771597","507771482","507771702","507771501","507771205","507771295","507771100","507771650","507771467",
"507771406","507771537","507771339","507771266","507771490","507771831","507771771","507771647","507771060","507771152",
"507771554","507771536","507771715","507771080","507771398","507771911","507771551","507771756","507771692","507771357",
"507771226","507771022","507771582","507771620","507771769","507771670","507771364","507771279","507771019","507771790",
"507771307","507771591","507771494","507771674","507771837","507771457","507771109","507771547","507771305","507771827",
"507771183","507771466","507771706","507771174","507771217","507771330","507771476","507771215","507771679","507771865",
"507771093","507771585","507771660","507771803","507771329","507771491","507771259","507771716","507771053","507771128",
"507771640","507771597","507771091","507771767","507771412","507771010","507771210","507771315","507771505","507771605",
}
local names = {"Dance","Robot","Floss","Twist","Whip","Wave","Point","Salute","Sit","Lay","Dab","Gangnam","Macarena","Harlem",
"Running Man","T-Pose","Cossack","Ballet","Sword","Karate","Boxing","Fencing","Taekwondo","Yoga","Breakdance","Moonwalk",
"Shuffle","Charleston","Tango","Waltz","Salsa","Mambo","Cha Cha","Rumba","Zumba","Hip Hop","Popping","Locking","Waacking",
"Voguing","Krumping","House","Industrial","Electro","Techno","Trance","Dubstep","Drum & Bass","Jazz","Tap","Modern",
"Contemporary","Lyrical","Musical","Ballroom","Swing","Lindy Hop","Jive","Boogie","Rock & Roll","Mosh","Circle Pit",
"Wall of Death","Clap","Cheer","Cry","Laugh","Shrug","Faint","Roar","Scream","Snap","Stomp","Thriller","Disco","Funky",
"Smooth","Cool","Attitude","Confused","Nervous","Shy","Sassy","Angry","Sad","Happy","Surprised","Disgusted","Fear",
"Pride","Love","Peace","Victory","Spin","Float","Kick","Punch","Jump","Slide","Backflip","Frontflip"}
local idx=0
for i=1,#names do
    idx=idx+1
    E(names[i].." "..idx, "rbxassetid://"..ids[((i-1)%#ids)+1])
end
for i=1,200 do
    idx=idx+1
    E("Extra Emote "..idx, "rbxassetid://"..ids[((i-1)%#ids)+1]..math.random(10,99))
end

--------------------------- ROLE / MAP HELPERS ---------------------------
local RoleColors = {
    Murderer=Color3.fromRGB(255,55,55), Sheriff=Color3.fromRGB(55,145,255), Innocent=Color3.fromRGB(55,230,100)
}
local MapTeleports = {
...
local function GetTargetRoot()
    ...
    return t.Character and t.Character:FindFirstChild("HumanoidRootPart")
end
...
local function clearESP(plr)
    if ESPObjects[plr] then
        for _, o in pairs(ESPObjects[plr]) do pcall(function() if o and o.Parent then o:Destroy() end end) end
        ESPObjects[plr] = nil
    end
end
...
local function createESP(plr)
    ...
    bb.Adornee = head; bb.Size = UDim2.new(0,280,0,100)
    ...
    nameL.Text = plr.Name.." ["..role.."]"; nameL.TextColor3 = color
    nameL.TextStrokeTransparency = 0.1; nameL.Font = Enum.Font.GothamBold
    nameL.TextSize = 14; nameL.Parent = bb

    local distL = Instance.new("TextLabel")
    distL.Size = UDim2.new(1,0,0.25,0); distL.Position = UDim2.new(0,0,0.4,0)
    distL.BackgroundTransparency = 1; distL.Text = "0"; distL.TextColor3 = color
    distL.Font = Enum.Font.Gotham; distL.TextSize = 12; distL.Parent = bb

    local hpL = Instance.new("TextLabel")
    hpL.Size = UDim2.new(1,0,0.2,0); hpL.Position = UDim2.new(0,0,0.65,0)
    hpL.BackgroundTransparency = 1; hpL.Text = "HP: 100"; hpL.TextColor3 = Color3.fromRGB(0,255,0)
    hpL.Font = Enum.Font.Gotham; hpL.TextSize = 12; hpL.Parent = bb

    local wpL = Instance.new("TextLabel")
    wpL.Size = UDim2.new(1,0,0.15,0); wpL.Position = UDim2.new(0,0,0.85,0)
    wpL.BackgroundTransparency = 1; wpL.Text = ""; wpL.TextColor3 = Color3.fromRGB(255,255,255)
    wpL.Font = Enum.Font.Gotham; wpL.TextSize = 11; wpL.Parent = bb
    ...
    if Features.ESP_Chams then
        ...
        hl.FillTransparency = 0.4; hl.OutlineTransparency = 0; hl.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop
        hl.Parent = char; o.Highlight = hl
    end
    if Features.ESP_Boxes then
        local box = Instance.new("BoxHandleAdornment")
        box.Adornee = root; box.Size = Vector3.new(4,6,2); box.Color3 = color
        box.Transparency = 0.5; box.AlwaysOnTop = true; box.Parent = root; o.Box = box
    end
...
local function Freeze(plr, s)
    local h = plr and plr.Character and plr.Character:FindFirstChildOfClass("Humanoid")
    if h then h.WalkSpeed = s and 0 or 16; h.JumpPower = s and 0 or 50 end
end
local function MakeInvis(plr, s)
    if not plr or not plr.Character then return end
...
local function SpinT(plr, s)
    local root = plr and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart"); if not root then return end
    local av = root:FindFirstChild("SpinAV")
    if s and not av then
        av = Instance.new("BodyAngularVelocity"); av.MaxTorque = Vector3.new(9e9,9e9,9e9)
        av.AngularVelocity = Vector3.new(0,20,0); av.Parent = root; av.Name = "SpinAV"
    elseif not s and av then av:Destroy() end
end
local function Explode(plr)
    local root = plr and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart"); if not root then return end
    local e = Instance.new("Explosion"); e.BlastRadius = 10; e.BlastPressure = 0; e.Position = root.Position; e.Parent = Workspace
    Debris:AddItem(e, 0.5)
end
local function PushPull(plr, dir)
    local root = plr and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart"); if not root then return end
    local bv = Instance.new("BodyVelocity"); bv.MaxForce = Vector3.new(9e9,9e9,9e9); bv.Velocity = dir*120; bv.Parent = root
    task.delay(0.5, function() if bv then bv:Destroy() end end)
end
local function SwapPos(plr)
    local r1, r2 = GetMyRoot(), GetTargetRoot()
    if not r1 or not r2 then return end
    local a, b = r1.Position, r2.Position
    r1.CFrame = CFrame.new(b); r2.CFrame = CFrame.new(a)
end
...
--------------------------- APPLY STATS / MOVEMENT ---------------------------
local function ApplyStats()
    pcall(function()
        local h = GetMyHum()
        if h then
            h.WalkSpeed = Features.SpeedBoost and 42 or Features.WalkSpeed
            h.JumpPower = Features.SuperJump and 120 or Features.JumpPower
        end
    end)
end
local function ApplyNoclip()
    pcall(function()
        ...
        local cc = Features.NoclipType == "None"
...
local function SetupFly()
    ...
    if Features.FlyType ~= "None" then
        BodyVel = Instance.new("BodyVelocity"); BodyVel.MaxForce = Vector3.new(9e9,9e9,9e9); BodyVel.Parent = root
        BodyGyro = Instance.new("BodyGyro"); BodyGyro.MaxTorque = Vector3.new(9e9,9e9,9e9); BodyGyro.P = 20000; BodyGyro.Parent = root
    end
end
    ...
    pcall(function()
        ...
        for _, p in ipairs(c:GetDescendants()) do
            if p:IsA("BasePart") or p:IsA("Decal") then
                if s then
                    if not originalTransparency[p] then originalTransparency[p] = p.Transparency end
                    p.Transparency = 1
                else
                    if originalTransparency[p] then p.Transparency = originalTransparency[p] end
                end
            end
        end
        if not s then table.clear(originalTransparency) end
    end)
end
...
local function OnChar()
    task.wait(0.5)
    ApplyStats(); ApplyNoclip()
    if Features.FlyType ~= "None" then SetupFly() end
    if Features.Invisible then SetInvis(true) end
...
--------------------------- MAIN LOOP ---------------------------
local lastHeavy, lastRole, lastScan, lastAnti, lastJump, lastAnnoy, lastFling, lastGlitch = 0,0,0,0,0,0,0,0
...
RunService.RenderStepped:Connect(function()
            ...
            for plr, o in pairs(ESPObjects) do
                if plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if o.HealthLabel then
                        ...
                        o.HealthLabel.Text = "HP: "..hp
                        o.HealthLabel.TextColor3 = hp > 50 and Color3.fromRGB(0,255,0) or Color3.fromRGB(255,0,0)
                    end
                    if o.WeaponLabel then
                        local t = plr.Character:FindFirstChildOfClass("Tool")
                        o.WeaponLabel.Text = t and ("Weapon: "..t.Name) or ""
                    end
    ...
    if Features.CFrameSpeed then
        ...
        if dir.Magnitude > 0 then root.CFrame += dir.Unit * Features.CFrameSpeedValue end
    end

    -- Infinite jump
    if Features.InfiniteJump and UserInputService:IsKeyDown(Enum.KeyCode.Space) and now-lastJump > 0.2 then
        lastJump = now
        pcall(function() hum:ChangeState(Enum.HumanoidStateType.Jumping) end)
    end
    -- Bunny hop
    if Features.BunnyHop and hum.FloorMaterial ~= Enum.Material.Air then
        pcall(function() hum:ChangeState(Enum.HumanoidStateType.Jumping) end)
    end
    -- Wallclimb
    ...
    -- Dash
    if Features.Dash and UserInputService:IsKeyDown(Enum.KeyCode.Space) and UserInputService:IsKeyDown(Enum.KeyCode.LeftShift) and now-lastJump > 0.5 then
        lastJump = now
        local dir = Camera.CFrame.LookVector * Features.DashPower
        root.AssemblyLinearVelocity = Vector3.new(dir.X, 0, dir.Z)
    end
    -- Teleport to mouse
    if Features.TeleportToMouse and UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then
    ...
    if Features.Orbit and Features.SelectedTarget then
        local t = GetTarget(); local tr = t and t.Character and t.Character:FindFirstChild("HumanoidRootPart")
        if tr then
            local a = (now * Features.OrbitSpeed) % (math.pi*2)
            root.CFrame = CFrame.new(tr.Position) * CFrame.Angles(0, a, 0) * CFrame.new(0, 2, Features.OrbitDist)
        end
    end
    if Features.LoopBehind and Features.SelectedTarget then
        local tr = GetTargetRoot()
        if tr then root.CFrame = tr.CFrame * CFrame.new(0, 0, 3.5) end
    end

    -- Carries
    if Features.Piggyback then
        local tr = GetTargetRoot(); if tr then root.CFrame = tr.CFrame * CFrame.new(0,3.1,0.2) end
    end
    if Features.FrontCarry then
        local tr = GetTargetRoot(); if tr then root.CFrame = tr.CFrame * CFrame.new(0,0,-3.1) end
    end
    if Features.SideCarry then
        local tr = GetTargetRoot(); if tr then root.CFrame = tr.CFrame * CFrame.new(2.7,0.4,0) end
    end
    ...
    -- Auto kill / shoot
    if Features.AutoKill and now-lastFling > 1.2 then
        lastFling = now
        local tool = char and char:FindFirstChildOfClass("Tool")
        if tool then pcall(function() tool:Activate() end) end
    end
    if Features.AutoShoot and now-lastFling > 0.4 then
        lastFling = now
        local tool = char and char:FindFirstChildOfClass("Tool")
        if tool then
            local n = string.lower(tool.Name)
    ...
    if Features.FlingTarget and Features.SelectedTarget and now-lastFling > 0.4 then
        lastFling = now
        local t = GetTarget(); if t then Fling(t) end
    end
    ...
    if t and t.Character then
        ...
        if Features.PlatformTarget then
            -- ensure a platform part under target
            local tr = t.Character:FindFirstChild("HumanoidRootPart")
            if tr then
                local existing = tr:FindFirstChild("ZuzyPlat")
        ...
        if Features.SlowMotionTarget then
    ...
    if Features.FreezeAll then
        for _, plr in ipairs(Players:GetPlayers()) do if plr ~= LocalPlayer then Freeze(plr, true) end end
    else
        for _, plr in ipairs(Players:GetPlayers()) do if plr ~= LocalPlayer then Freeze(plr, false) end end
    end
    if Features.SpinAll then
        for _, plr in ipairs(Players:GetPlayers()) do if plr ~= LocalPlayer then SpinT(plr, true) end end
    else
        for _, plr in ipairs(Players:GetPlayers()) do if plr ~= LocalPlayer then SpinT(plr, false) end end
    end

    -- Rainbow
    if Features.RainbowSelf then
        local hue = now % 1
...
Home:CreateButton({ name = "Status", callback = function()
    local info = string.format("User: %s (%d)\nTier: %s\nTrusted: %s\nVersion: %s",
        MY_USERNAME, MY_UID, acquiredTier, tostring(userRow.is_trusted or false), VERSION)
    Rayfield:Notify({ title = "Status", content = info, duration = 8 })
end })
Home:CreateButton({ name = "Copy Discord / Credits", callback = function()
    if setclipboard then setclipboard(CREDITS) end
end })
...
Home:CreateButton({ name = "Request Developer", callback = function()
    if #DevReason < 3 then return end
    http("POST", DISCORD_WEBHOOK, { ["Content-Type"]="application/json" }, { content = "**Developer Request**\nUser: "..MY_USERNAME.." ("..MY_UID..")\nReason: "..DevReason })
    Rayfield:Notify({ title = "Sent", content = "Request sent.", duration = 5 })
end })

-- SETTINGS
local Settings = window:CreateTab({ name = "Settings" })
Settings:CreateSection({ name = "Information" })
Settings:CreateButton({ name = "Version: "..VERSION, callback = function() Rayfield:Notify({ title="Version", content=VERSION.."\n"..CREDITS, duration=8 }) end })
Settings:CreateButton({ name = "Credits", callback = function() Rayfield:Notify({ title="Credits", content=CREDITS, duration=10 }) end })
...
Settings:CreateToggle({
    ...
    callback = function(v)
        ...
        if v then
            -- opt-in gives trusted flag if tier >= basic
            if HasTier("basic") then
                sbPatch("zuzify_users", "user_id=eq."..MY_UID, { is_trusted = true })
                userRow.is_trusted = true
                Rayfield:Notify({ title="Trusted", content="You are now a Trusted User! Extra features unlocked.", duration=8 })
            else
                Rayfield:Notify({ title="Trusted", content="Opted in. Buy a key to become officially Trusted.", duration=8 })
            end
        end
    end,
})
...
Settings:CreateDropdown({
    ...
    callback = function(v) local n = DD(v); if Themes[n] then window:ChangeTheme(Themes[n]) end end,
})
...
PlayersTab:CreateButton({ name = "Refresh Players", callback = function() end })
...
Troll:CreateButton({ name = "Push Away", callback = function()
    local t = GetTarget(); local r1, r2 = GetMyRoot(), GetTargetRoot()
    if t and r1 and r2 then PushPull(t, (r1.Position - r2.Position).Unit) end
end })
Troll:CreateButton({ name = "Pull Toward", callback = function()
    local t = GetTarget(); local r1, r2 = GetMyRoot(), GetTargetRoot()
    if t and r1 and r2 then PushPull(t, (r2.Position - r1.Position).Unit) end
end })
...
EmoteTab:CreateDropdown({ name = "Emote", options = emoteNames, callback = function(v)
    local n = DD(v)
    for _, e in ipairs(EmoteList) do if e.Name == n then Features.SelectedEmote = e.ID end end
end })
...
EmoteTab:CreateButton({ name = "Play on Self", callback = function()
    if Features.SelectedEmote then clearEmotes(); PlayEmote(LocalPlayer, Features.SelectedEmote) end
end })
EmoteTab:CreateButton({ name = "Play on Target", callback = function()
    local t = GetTarget(); if t and Features.SelectedEmote then clearEmotes(); PlayEmote(t, Features.SelectedEmote) end
end })
EmoteTab:CreateButton({ name = "Stop Emotes", callback = clearEmotes })
EmoteTab:CreateButton({ name = "Random Emote", callback = function()
    if #EmoteList > 0 then
        local e = EmoteList[math.random(1, #EmoteList)]
        clearEmotes(); PlayEmote(LocalPlayer, e.ID)
    end
end })
...
if IS_OWNER or IS_DEV then
    ...
    local function refreshOnline()
        ...
        for _, s in ipairs(data or {}) do
            if s.show_username and s.username then
                table.insert(lines, s.username .. " ("..s.user_id..")")
            else
                table.insert(lines, "Anonymous #"..tostring(s.user_id):sub(-4))
            end
        end
        Rayfield:Notify({ title = "Online: "..cnt, content = table.concat(lines, "\n"), duration = 10 })
    end
    OwnerTab:CreateButton({ name = "Refresh Online Users", callback = refreshOnline })
    ...
    OwnerTab:CreateButton({ name = "Full User Stats", callback = function()
        ...
        Rayfield:Notify({ title = "Stats", content = "Total: "..total.."\nPaid: "..paid.."\nTrusted: "..trusted.."\nBanned: "..banned, duration = 10 })
    end })

    -- Key generation (owner + developer)
    ...
    OwnerTab:CreateButton({ name = "Generate Key", callback = function()
        ...
        Rayfield:Notify({ title = "Key Created", content = key, duration = 12 })
    ...
    OwnerTab:CreateButton({ name = "List Active Keys", callback = function()
        ...
        Rayfield:Notify({ title = "Keys", content = table.concat(s, "\n"), duration = 12 })
    end })
    ...
    OwnerTab:CreateButton({ name = "Kick User", callback = function()
        ...
        Rayfield:Notify({ title = "Kick Queued", content = "User will be kicked within 30s.", duration = 6 })
    end })
    local banMins = 60
    ...
    OwnerTab:CreateButton({ name = "Ban User", callback = function()
        ...
        Rayfield:Notify({ title = "Ban Applied", content = "User banned.", duration = 6 })
    end })
    ...
    OwnerTab:CreateButton({ name = "Force Refresh All Licenses", callback = function()
        sbPatch("zuzify_users", "user_id=gt.0", { last_seen = DateTime.now():ToIsoDate() })
        Rayfield:Notify({ title = "OK", content = "Pinged all users.", duration = 5 })
    end })

    -- Dev tools
    OwnerTab:CreateSection({ name = "Dev Tools" })
    OwnerTab:CreateButton({ name = "Set My Tier → Premium", callback = function()
        sbPatch("zuzify_users", "user_id=eq."..MY_UID, { tier = "premium", is_paid = true })
        Rayfield:Notify({ title = "Tier", content = "Set to premium. Rejoin to apply.", duration = 6 })
    end })
    OwnerTab:CreateButton({ name = "Wipe My Session", callback = function()
        if sessionId then
            http("DELETE", SUPABASE_URL.."/rest/v1/zuzify_sessions?id=eq."..sessionId, sbHeaders())
        end
    end })
end
...
Rayfield:Notify({
    ...
    content = "Welcome "..MY_USERNAME.."! Tier: "..string.upper(acquiredTier)..
