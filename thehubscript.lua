--[[
    ZuzifyRBX - Gen2 Mega Build
    More MM2 / Movement / Troll / Cheese + 6 extras
]]
...
local window = Rayfield:CreateWindow({
    ...
    subtitle = "Mega Build • More Everything",
    theme = Themes["OLED Dark"],
    configuration = { autoSave = true, autoLoad = true, fileName = "ZuzifyRBX_Mega" },
})
...
local Features = {
    ...
    MurderWalk = false, FollowSheriff = false, AutoEmote = false,
    -- Troll
    ...
    LoopBehind = false, SkyPlatform = false, AnnoyAura = false, BringAll = false,
    PlatformSpam = false, BounceTarget = false,
    -- Cheese
    ShowCodes = false, ShowKeys = false, ShowCheese = false, ShowDoors = false,
    RatESP = false, AutoCheese = false, CheeseRadar = false,
    -- Anti
...
local function CreateESP(plr)
    ...
    if Features.ESP_Names or Features.ESP_Distance then
        ...
        distL.TextSize = 12
        distL.Parent = bb
        objects.Billboard, objects.NameLabel, objects.DistLabel = bb, nameL, distL
    end
...
RunService.RenderStepped:Connect(function()
    ...
    if Features.Orbit and Features.SelectedTarget and root then
        local t = Players:FindFirstChild(Features.SelectedTarget)
...
Troll:CreateSection({ name = "Carry / Stick" })
...
Troll:CreateToggle({ name = "Sky Platform", callback = function(v) Features.SkyPlatform = v end })
Troll:CreateButton({
    name = "Fling Selected Once",
    callback = function()
        local t = Features.SelectedTarget and Players:FindFirstChild(Features.SelectedTarget)
        if t then Fling(t) else Notify("Troll", "Pick a player first") end
    end,
})
...
Extra:CreateSection({ name = "6+ Features" })
Extra:CreateToggle({ name = "Orbit Target", callback = function(v) Features.Orbit = v end })
Extra:CreateToggle({ name = "Loop Behind", callback = function(v) Features.LoopBehind = v end })
Extra:CreateToggle({ name = "Sky Platform", callback = function(v) Features.SkyPlatform = v end })
...
Extra:CreateToggle({ name = "Annoy Aura", callback = function(v) Features.AnnoyAura = v end })
...
Notify("ZuzifyRBX", "Mega Build | tai loves sofia!!!!!!!")
print("ZuzifyRBX Mega | Rank:", CurrentRank)
